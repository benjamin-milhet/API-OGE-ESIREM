from viewState import ViewState

import requests
import re
import json
import smtplib

class OGE:
    OGE_ESIREM = "https://casiut21.u-bourgogne.fr/cas-esirem/login?service=https%3A%2F%2Fiutdijon.u-bourgogne.fr%2Foge-esirem%2F"
    OGE_DETAILS = "https://iutdijon.u-bourgogne.fr/oge-esirem/stylesheets/etu/detailsEtu.xhtml"

    def __init__(self, login, password):
        self.login = login
        self.password = password
        self.session = requests.session()
    #Permet de se connecter a mon compte OGE-ESIREM
    def connexion(self):
        dataConnexion = {"username": self.login,
                "password": self.password,
                "execution": "e236c444-5ef9-4a35-a7dd-1afcb6a444d2_",
                "service": "https://iutdijon.u-bourgogne.fr/oge-esirem/",
                "_eventId": "submit"
                }
        response = self.session.post(self.OGE_ESIREM, data = dataConnexion)

    #Permet de recuperer le nombre de mote de chacune de mes matieres
    def getNombreNote(self):
        VS = ViewState(self.OGE_DETAILS, self.session, re)
        id, viewS = VS.getViewState()
        data = {"javax.faces.partial.execute": "@all",
                    "javax.faces.partial.render": "mainFormDetailNote",
                    id: id,
                    id + "_menuid": str(0),
                    "mainFormDetailNote_SUBMIT": "1",
                    "javax.faces.ViewState": viewS}
        r = self.session.post(self.OGE_DETAILS, data=data, headers={'referer':self.OGE_DETAILS, 'Faces-Request': 'partial/ajax'})
        matieresResult = re.findall(
            r"request\('(.*?):elpLink',event(?:.*?)>- (.*?)</a>(?:.*?)font-style:italic;(?:.*?)\">\((?:(?:([0-9]+) note(?:s|))|(?:pas de notes))\)</font>",
            r.text, re.DOTALL)

        listMatiereFichier, listNoteFichier = self.getNotesMatieres()
        listMatiere = []
        nbNote = []
        listMatiereNom = []
        totalNote = 0
        i = 0;
        for matiere in matieresResult:
            print(matiere[1], matiere[2])
            listMatiere.append(matiere[1])
            nbNote.append(matiere[2])

            if (listMatiereFichier[i] == matiere[1]):
                if ((listNoteFichier[i] != "") and (matiere[2] != "")):
                    if (int(listNoteFichier[i]) < int(matiere[2])):
                        listMatiereNom.append(listMatiereFichier[i])
                if ((listNoteFichier[i] == "") and (matiere[2] != "")):
                    listMatiereNom.append(listMatiereFichier[i])

            if (matiere[2] != ''):
                totalNote += int(matiere[2])
            i+=1

        if (totalNote > self.getNoteTotal()):
            self.envoyerMail(listMatiereNom)

        self.write(self.convertToJSON(listMatiere, nbNote))

    #Permet d'ecrire dans le fichier data.json
    def write(self, matieres):
        with open("/home/pi/Documents/OGE-API-ESIREM/data.json", "w") as fichier:
            fichier.write(matieres)

    #Permet de lire le contenu du fichier data.json
    def read(self):
        with open('/home/pi/Documents/OGE-API-ESIREM/data.json') as json_data:
            data_dict = json.load(json_data)
        return data_dict

    def convertToJSON(self, matieres, nbNote):
        dictionnaire = {k: v for k, v in zip(matieres, nbNote)}
        cJson = json.dumps(dictionnaire)
        return cJson

    def getNoteTotal(self):
        totalNote = 0

        for cle, valeur in self.read().items():
            if (valeur != ''):
                totalNote += int(valeur)

        return totalNote

    def getNotesMatieres(self):
        listMatiere = []
        listNote = []
        for cle, valeur in self.read().items():
            listMatiere.append(cle)
            listNote.append(valeur)
        return listMatiere, listNote

    #Permet d'envoyer un mail des lors qu'il y a une nouvelle note
    def envoyerMail(self, matieres):
        with open("/home/pi/Documents/OGE-API-ESIREM/passwordGmail.txt", "r") as fichier:
            SMTP_PASSWORD = fichier.read()
            
        SMTP_SERVER = "smtp.gmail.com"
        SMTP_PORT = 587
        SMTP_USERNAME = "milhetbenjamin@gmail.com"
        EMAIL_FROM = "milhetbenjamin@gmail.com"
        EMAIL_TO = "benjamin.milhet@outlook.fr"
        EMAIL_SUBJECT = "Nouvelle note sur OGE"
        EMAIL_MESSAGE = ""
        for matiere in matieres:
            EMAIL_MESSAGE += "Vous avez une nouvelle note en " + matiere + ".\n"
        EMAIL_MESSAGE = ["\n".join(msg.replace(u'\xe9', u' ')) for msg in EMAIL_MESSAGE]
        EMAIL_MESSAGE = ''.join(EMAIL_MESSAGE)
        
        s = smtplib.SMTP(SMTP_SERVER, SMTP_PORT)
        s.starttls()
        s.login(SMTP_USERNAME, SMTP_PASSWORD)
        message = 'Subject: {}\n\n{}'.format(EMAIL_SUBJECT, EMAIL_MESSAGE)
        s.sendmail(EMAIL_FROM, EMAIL_TO, message)
        s.quit()





